  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-zinc-900">Fahrzeuge</h1>
        <Link
          href="/fahrzeuge/neu"
      ...
      {loading ? (
        ...
        <p className="text-sm text-zinc-500">Keine Fahrzeuge gefunden.</p>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {vehicles.map((v) => (
            ...
            >
              <p className="text-lg font-semibold text-zinc-900">{v.kennzeichen}</p>
              <p className="text-sm text-zinc-600">
                {v.marke} {v.modell} {v.baujahr ? `(${v.baujahr})` : ""}
              </p>
              {v.kundeName && <p className="mt-1 text-sm text-zinc-500">Kunde: {v.kundeName}</p>}
